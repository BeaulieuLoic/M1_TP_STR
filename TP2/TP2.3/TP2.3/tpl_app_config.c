...
/* Event EvOff */
#define EvOff_mask 1
CONST(EventMaskType, AUTOMATIC) EvOff = EvOff_mask;

/*=============================================================================
 * Declaration of processes IDs
 */

/* Task task_boutondroit identifier */
#define task_boutondroit_id 0
CONST(TaskType, AUTOMATIC) task_boutondroit = task_boutondroit_id;

/* Task task_distance identifier */
#define task_distance_id 1
CONST(TaskType, AUTOMATIC) task_distance = task_distance_id;

/* Task task_pression identifier */
#define task_pression_id 2
CONST(TaskType, AUTOMATIC) task_pression = task_pression_id;

/* Task task_systime identifier */
#define task_systime_id 3
CONST(TaskType, AUTOMATIC) task_systime = task_systime_id;
...
#define alarm_distance_id 0
CONST(AlarmType, AUTOMATIC) alarm_distance = alarm_distance_id;

/* Alarm alarm_pression identifier */
#define alarm_pression_id 1
CONST(AlarmType, AUTOMATIC) alarm_pression = alarm_pression_id;

/* Alarm alarm_systime identifier */
#define alarm_systime_id 2
CONST(AlarmType, AUTOMATIC) alarm_systime = alarm_systime_id;
...
CONST(tpl_tick, OS_CONST) OSMAXALLOWEDVALUE = 3000;
 ...
 */
/*-----------------------------------------------------------------------------
 * Task task_boutondroit descriptor
 */
#define APP_Task_task_boutondroit_START_SEC_CODE
#include "tpl_memmap.h"
/*
 * Task task_boutondroit function prototype
 */
FUNC(void, OS_APPL_CODE) task_boutondroit_function(void);
#define APP_Task_task_boutondroit_STOP_SEC_CODE
#include "tpl_memmap.h"
 ...
 */
/*
 * Task task_boutondroit stack
 */
#define APP_Task_task_boutondroit_START_SEC_STACK
#include "tpl_memmap.h"
VAR(tpl_stack_word, OS_APPL_DATA) task_boutondroit_stack_zone[500/sizeof(tpl_stack_word)];
#define APP_Task_task_boutondroit_STOP_SEC_STACK
#include "tpl_memmap.h"

#define task_boutondroit_STACK {task_boutondroit_stack_zone, 500}

/*
 * Task task_boutondroit context
 */
#define OS_START_SEC_VAR_NOINIT_32BIT
#include "tpl_memmap.h"
VAR(arm_context, OS_VAR) task_boutondroit_int_context;

#define task_boutondroit_CONTEXT &task_boutondroit_int_context
...
/*
 * Static descriptor of task task_boutondroit
 */
CONST(tpl_proc_static, OS_CONST) task_boutondroit_task_stat_desc = {
  /* context                  */  task_boutondroit_CONTEXT,
  /* stack                    */  task_boutondroit_STACK,
  /* entry point (function)   */  task_boutondroit_function,
  /* internal ressource       */  NULL,
  /* task id                  */  task_boutondroit_id,
#if WITH_OSAPPLICATION == YES
  /* OS application id        */  
#endif
...
#include "tpl_memmap.h"
/*
 * Dynamic descriptor of task task_boutondroit
 */
VAR(tpl_proc, OS_VAR) task_boutondroit_task_desc = {
  /* resources                      */  NULL,
...
/*
 * Event structure of task task_boutondroit
 */
VAR(tpl_task_events, OS_VAR) task_boutondroit_task_evts = {
  /* event set  */ 0,
  /* event wait */ 0
};
...
/*-----------------------------------------------------------------------------
 * Task task_distance descriptor
 */
#define APP_Task_task_distance_START_SEC_CODE
#include "tpl_memmap.h"
/*
 * Task task_distance function prototype
 */
FUNC(void, OS_APPL_CODE) task_distance_function(void);
#define APP_Task_task_distance_STOP_SEC_CODE
#include "tpl_memmap.h"
 ...
 */
/*
 * Task task_distance stack
 */
#define APP_Task_task_distance_START_SEC_STACK
#include "tpl_memmap.h"
VAR(tpl_stack_word, OS_APPL_DATA) task_distance_stack_zone[500/sizeof(tpl_stack_word)];
#define APP_Task_task_distance_STOP_SEC_STACK
#include "tpl_memmap.h"

#define task_distance_STACK {task_distance_stack_zone, 500}

/*
 * Task task_distance context
 */
#define OS_START_SEC_VAR_NOINIT_32BIT
#include "tpl_memmap.h"
VAR(arm_context, OS_VAR) task_distance_int_context;

#define task_distance_CONTEXT &task_distance_int_context
...
/*
 * Static descriptor of task task_distance
 */
CONST(tpl_proc_static, OS_CONST) task_distance_task_stat_desc = {
  /* context                  */  task_distance_CONTEXT,
  /* stack                    */  task_distance_STACK,
  /* entry point (function)   */  task_distance_function,
  /* internal ressource       */  NULL,
  /* task id                  */  task_distance_id,
#if WITH_OSAPPLICATION == YES
  /* OS application id        */  
#endif
  /* task base priority       */  1,
...
#include "tpl_memmap.h"
/*
 * Dynamic descriptor of task task_distance
 */
VAR(tpl_proc, OS_VAR) task_distance_task_desc = {
  /* resources                      */  NULL,
#if WITH_MEMORY_PROTECTION == YES
  /* if > 0 the process is trusted  */  0,    
#endif /* WITH_MEMORY_PROTECTION */
...
/*-----------------------------------------------------------------------------
 * Task task_pression descriptor
 */
#define APP_Task_task_pression_START_SEC_CODE
#include "tpl_memmap.h"
/*
 * Task task_pression function prototype
 */
FUNC(void, OS_APPL_CODE) task_pression_function(void);
#define APP_Task_task_pression_STOP_SEC_CODE
#include "tpl_memmap.h"
 ...
 */
/*
 * Task task_pression stack
 */
#define APP_Task_task_pression_START_SEC_STACK
#include "tpl_memmap.h"
VAR(tpl_stack_word, OS_APPL_DATA) task_pression_stack_zone[500/sizeof(tpl_stack_word)];
#define APP_Task_task_pression_STOP_SEC_STACK
#include "tpl_memmap.h"

#define task_pression_STACK {task_pression_stack_zone, 500}

/*
 * Task task_pression context
 */
#define OS_START_SEC_VAR_NOINIT_32BIT
#include "tpl_memmap.h"
VAR(arm_context, OS_VAR) task_pression_int_context;

#define task_pression_CONTEXT &task_pression_int_context
...
/*
 * Static descriptor of task task_pression
 */
CONST(tpl_proc_static, OS_CONST) task_pression_task_stat_desc = {
  /* context                  */  task_pression_CONTEXT,
  /* stack                    */  task_pression_STACK,
  /* entry point (function)   */  task_pression_function,
  /* internal ressource       */  NULL,
  /* task id                  */  task_pression_id,
#if WITH_OSAPPLICATION == YES
  /* OS application id        */  
#endif
  /* task base priority       */  1,
...
#include "tpl_memmap.h"
/*
 * Dynamic descriptor of task task_pression
 */
VAR(tpl_proc, OS_VAR) task_pression_task_desc = {
  /* resources                      */  NULL,
#if WITH_MEMORY_PROTECTION == YES
  /* if > 0 the process is trusted  */  0,    
#endif /* WITH_MEMORY_PROTECTION */
...
/*-----------------------------------------------------------------------------
 * Task task_systime descriptor
 */
#define APP_Task_task_systime_START_SEC_CODE
#include "tpl_memmap.h"
/*
 * Task task_systime function prototype
 */
FUNC(void, OS_APPL_CODE) task_systime_function(void);
#define APP_Task_task_systime_STOP_SEC_CODE
#include "tpl_memmap.h"
 ...
 */
/*
 * Task task_systime stack
 */
#define APP_Task_task_systime_START_SEC_STACK
#include "tpl_memmap.h"
VAR(tpl_stack_word, OS_APPL_DATA) task_systime_stack_zone[500/sizeof(tpl_stack_word)];
#define APP_Task_task_systime_STOP_SEC_STACK
#include "tpl_memmap.h"

#define task_systime_STACK {task_systime_stack_zone, 500}

/*
 * Task task_systime context
 */
#define OS_START_SEC_VAR_NOINIT_32BIT
#include "tpl_memmap.h"
VAR(arm_context, OS_VAR) task_systime_int_context;

#define task_systime_CONTEXT &task_systime_int_context
...
/*
 * Static descriptor of task task_systime
 */
CONST(tpl_proc_static, OS_CONST) task_systime_task_stat_desc = {
  /* context                  */  task_systime_CONTEXT,
  /* stack                    */  task_systime_STACK,
  /* entry point (function)   */  task_systime_function,
  /* internal ressource       */  NULL,
  /* task id                  */  task_systime_id,
#if WITH_OSAPPLICATION == YES
  /* OS application id        */  
#endif
  /* task base priority       */  1,
...
#include "tpl_memmap.h"
/*
 * Dynamic descriptor of task task_systime
 */
VAR(tpl_proc, OS_VAR) task_systime_task_desc = {
  /* resources                      */  NULL,
#if WITH_MEMORY_PROTECTION == YES
  /* if > 0 the process is trusted  */  0,    
#endif /* WITH_MEMORY_PROTECTION */
...
tpl_stat_proc_table[TASK_COUNT+ISR_COUNT+1] = {
  &task_boutondroit_task_stat_desc,
  &task_distance_task_stat_desc,
  &task_pression_task_stat_desc,
  &task_systime_task_stat_desc,
...
tpl_dyn_proc_table[TASK_COUNT+ISR_COUNT+1] = {
  &task_boutondroit_task_desc,
  &task_distance_task_desc,
  &task_pression_task_desc,
  &task_systime_task_desc,
 ...
 *
 * This alarm does the activation of task task_distance
 */
...
/*-----------------------------------------------------------------------------
 * Alarm alarm_pression descriptor
 *
 * This alarm does the activation of task task_pression
 */
...
CONST(tpl_task_activation_action, OS_CONST) alarm_pression_action = {
  {
    /* action function  */  tpl_action_activate_task
  },
  /* task id            */  task_pression_id
...
CONST(tpl_alarm_static, OS_CONST) alarm_pression_static = {
  {
    /* pointer to counter           */  &SystemCounter_counter_desc,
    /* pointer to the expiration    */  tpl_raise_alarm
#if (WITH_TRACE == YES)
    /* id of the alarm for tracing  */  , alarm_pression_id
#endif
#if WITH_OSAPPLICATION == YES
    /* OS application id            */  , 
#endif
  },
  /* action of the alarm  */  (tpl_action *)&alarm_pression_action
};
...
VAR(tpl_time_obj, OS_VAR) alarm_pression_alarm_desc = {
    /* pointer to the static part   */  (tpl_time_obj_static *)&alarm_pression_static,
    /* next alarm                   */  NULL,
    /* prev alarm                   */  NULL,
    /* cycle                        */  3000,
    /* date                         */  100,
...
/*-----------------------------------------------------------------------------
 * Alarm alarm_systime descriptor
 *
 * This alarm does the activation of task task_systime
 */
...
CONST(tpl_task_activation_action, OS_CONST) alarm_systime_action = {
  {
    /* action function  */  tpl_action_activate_task
  },
  /* task id            */  task_systime_id
...
CONST(tpl_alarm_static, OS_CONST) alarm_systime_static = {
  {
    /* pointer to counter           */  &SystemCounter_counter_desc,
    /* pointer to the expiration    */  tpl_raise_alarm
#if (WITH_TRACE == YES)
    /* id of the alarm for tracing  */  , alarm_systime_id
#endif
#if WITH_OSAPPLICATION == YES
    /* OS application id            */  , 
#endif
  },
  /* action of the alarm  */  (tpl_action *)&alarm_systime_action
};
...
VAR(tpl_time_obj, OS_VAR) alarm_systime_alarm_desc = {
    /* pointer to the static part   */  (tpl_time_obj_static *)&alarm_systime_static,
    /* next alarm                   */  NULL,
    /* prev alarm                   */  NULL,
    /* cycle                        */  1000,
    /* date                         */  100,
...
CONSTP2VAR(tpl_time_obj, AUTOMATIC, OS_APPL_DATA)
  ...
  &alarm_pression_alarm_desc,
  &alarm_systime_alarm_desc
};
#define OS_STOP_SEC_CONST_UNSPECIFIED
#include "tpl_memmap.h"
