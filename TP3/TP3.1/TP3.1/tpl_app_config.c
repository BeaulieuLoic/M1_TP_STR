...
/* Resource res_correction */
#define res_correction_id 0
CONST(ResourceType, AUTOMATIC) res_correction = res_correction_id;

/* Resource res_distance */
#define res_distance_id 1
CONST(ResourceType, AUTOMATIC) res_distance = res_distance_id;

/*=============================================================================
 * Declaration of processes IDs
 */

/* Task task_detection_contact identifier */
#define task_detection_contact_id 0
CONST(TaskType, AUTOMATIC) task_detection_contact = task_detection_contact_id;

/* Task task_detection_distance identifier */
#define task_detection_distance_id 1
CONST(TaskType, AUTOMATIC) task_detection_distance = task_detection_distance_id;

/* Task task_navigation identifier */
#define task_navigation_id 2
CONST(TaskType, AUTOMATIC) task_navigation = task_navigation_id;
...
/* Alarm alarm_navigation identifier */
#define alarm_navigation_id 2
CONST(AlarmType, AUTOMATIC) alarm_navigation = alarm_navigation_id;
...
#include "tpl_memmap.h"
/*-----------------------------------------------------------------------------
 * Resource descriptor of resource res_correction
 *
 * Tasks which use this resource :
 * task_detection_contact
 * task_detection_distance
 */
VAR(tpl_resource, OS_VAR) res_correction_rez_desc = {
...
#include "tpl_memmap.h"
/*-----------------------------------------------------------------------------
 * Resource descriptor of resource res_distance
 *
 * Tasks which use this resource :
 * task_detection_contact
 * task_detection_distance
 */
VAR(tpl_resource, OS_VAR) res_distance_rez_desc = {
  /* ceiling priority of the resource */  1,
  /* owner previous priority          */  0,
  /* owner of the resource            */  INVALID_PROC_ID,
#if WITH_OSAPPLICATION == YES
...
tpl_resource_table[RESOURCE_COUNT] = {
  &res_correction_rez_desc,
  &res_distance_rez_desc,
  &res_sched_rez_desc  
};
 ...
 */
/*-----------------------------------------------------------------------------
 * Task task_detection_contact descriptor
 */
#define APP_Task_task_detection_contact_START_SEC_CODE
#include "tpl_memmap.h"
/*
 * Task task_detection_contact function prototype
 */
FUNC(void, OS_APPL_CODE) task_detection_contact_function(void);
#define APP_Task_task_detection_contact_STOP_SEC_CODE
#include "tpl_memmap.h"
 ...
 */
/*
 * Task task_detection_contact stack
 */
#define APP_Task_task_detection_contact_START_SEC_STACK
#include "tpl_memmap.h"
VAR(tpl_stack_word, OS_APPL_DATA) task_detection_contact_stack_zone[500/sizeof(tpl_stack_word)];
#define APP_Task_task_detection_contact_STOP_SEC_STACK
#include "tpl_memmap.h"

#define task_detection_contact_STACK {task_detection_contact_stack_zone, 500}

/*
 * Task task_detection_contact context
 */
#define OS_START_SEC_VAR_NOINIT_32BIT
#include "tpl_memmap.h"
VAR(arm_context, OS_VAR) task_detection_contact_int_context;

#define task_detection_contact_CONTEXT &task_detection_contact_int_context
...
/*
 * Resources used by task task_detection_contact
 *
 * res_distance, res_correction
 */

/*
 * Static descriptor of task task_detection_contact
 */
CONST(tpl_proc_static, OS_CONST) task_detection_contact_task_stat_desc = {
  /* context                  */  task_detection_contact_CONTEXT,
  /* stack                    */  task_detection_contact_STACK,
  /* entry point (function)   */  task_detection_contact_function,
  /* internal ressource       */  NULL,
  /* task id                  */  task_detection_contact_id,
#if WITH_OSAPPLICATION == YES
  /* OS application id        */  
#endif
...
#include "tpl_memmap.h"
/*
 * Dynamic descriptor of task task_detection_contact
 */
VAR(tpl_proc, OS_VAR) task_detection_contact_task_desc = {
  /* resources                      */  NULL,
...
/*-----------------------------------------------------------------------------
 * Task task_detection_distance descriptor
 */
#define APP_Task_task_detection_distance_START_SEC_CODE
#include "tpl_memmap.h"
/*
 * Task task_detection_distance function prototype
 */
FUNC(void, OS_APPL_CODE) task_detection_distance_function(void);
#define APP_Task_task_detection_distance_STOP_SEC_CODE
#include "tpl_memmap.h"
 ...
 */
/*
 * Task task_detection_distance stack
 */
#define APP_Task_task_detection_distance_START_SEC_STACK
#include "tpl_memmap.h"
VAR(tpl_stack_word, OS_APPL_DATA) task_detection_distance_stack_zone[500/sizeof(tpl_stack_word)];
#define APP_Task_task_detection_distance_STOP_SEC_STACK
#include "tpl_memmap.h"

#define task_detection_distance_STACK {task_detection_distance_stack_zone, 500}

/*
 * Task task_detection_distance context
 */
#define OS_START_SEC_VAR_NOINIT_32BIT
#include "tpl_memmap.h"
VAR(arm_context, OS_VAR) task_detection_distance_int_context;

#define task_detection_distance_CONTEXT &task_detection_distance_int_context
...
/*
 * Resources used by task task_detection_distance
 *
 * res_distance, res_correction
 */

/*
 * Static descriptor of task task_detection_distance
 */
CONST(tpl_proc_static, OS_CONST) task_detection_distance_task_stat_desc = {
  /* context                  */  task_detection_distance_CONTEXT,
  /* stack                    */  task_detection_distance_STACK,
  /* entry point (function)   */  task_detection_distance_function,
  /* internal ressource       */  NULL,
  /* task id                  */  task_detection_distance_id,
#if WITH_OSAPPLICATION == YES
  /* OS application id        */  
#endif
  /* task base priority       */  1,
...
#include "tpl_memmap.h"
/*
 * Dynamic descriptor of task task_detection_distance
 */
VAR(tpl_proc, OS_VAR) task_detection_distance_task_desc = {
  /* resources                      */  NULL,
#if WITH_MEMORY_PROTECTION == YES
  /* if > 0 the process is trusted  */  0,    
#endif /* WITH_MEMORY_PROTECTION */
...
/*-----------------------------------------------------------------------------
 * Task task_navigation descriptor
 */
#define APP_Task_task_navigation_START_SEC_CODE
#include "tpl_memmap.h"
/*
 * Task task_navigation function prototype
 */
FUNC(void, OS_APPL_CODE) task_navigation_function(void);
#define APP_Task_task_navigation_STOP_SEC_CODE
#include "tpl_memmap.h"
 ...
 */
/*
 * Task task_navigation stack
 */
#define APP_Task_task_navigation_START_SEC_STACK
#include "tpl_memmap.h"
VAR(tpl_stack_word, OS_APPL_DATA) task_navigation_stack_zone[500/sizeof(tpl_stack_word)];
#define APP_Task_task_navigation_STOP_SEC_STACK
#include "tpl_memmap.h"

#define task_navigation_STACK {task_navigation_stack_zone, 500}

/*
 * Task task_navigation context
 */
#define OS_START_SEC_VAR_NOINIT_32BIT
#include "tpl_memmap.h"
VAR(arm_context, OS_VAR) task_navigation_int_context;

#define task_navigation_CONTEXT &task_navigation_int_context
...
/*
 * Static descriptor of task task_navigation
 */
CONST(tpl_proc_static, OS_CONST) task_navigation_task_stat_desc = {
  /* context                  */  task_navigation_CONTEXT,
  /* stack                    */  task_navigation_STACK,
  /* entry point (function)   */  task_navigation_function,
  /* internal ressource       */  NULL,
  /* task id                  */  task_navigation_id,
#if WITH_OSAPPLICATION == YES
  /* OS application id        */  
#endif
  /* task base priority       */  1,
...
#include "tpl_memmap.h"
/*
 * Dynamic descriptor of task task_navigation
 */
VAR(tpl_proc, OS_VAR) task_navigation_task_desc = {
  /* resources                      */  NULL,
#if WITH_MEMORY_PROTECTION == YES
  /* if > 0 the process is trusted  */  0,    
#endif /* WITH_MEMORY_PROTECTION */
...
tpl_stat_proc_table[TASK_COUNT+ISR_COUNT+1] = {
  &task_detection_contact_task_stat_desc,
  &task_detection_distance_task_stat_desc,
  &task_navigation_task_stat_desc,
...
tpl_dyn_proc_table[TASK_COUNT+ISR_COUNT+1] = {
  &task_detection_contact_task_desc,
  &task_detection_distance_task_desc,
  &task_navigation_task_desc,
 ...
 *
 * This alarm does the activation of task task_detection_contact
 */
 ...
 *
 * This alarm does the activation of task task_detection_distance
 */
...
/*-----------------------------------------------------------------------------
 * Alarm alarm_navigation descriptor
 *
 * This alarm does the activation of task task_navigation
 */
...
CONST(tpl_task_activation_action, OS_CONST) alarm_navigation_action = {
  {
    /* action function  */  tpl_action_activate_task
  },
  /* task id            */  task_navigation_id
...
CONST(tpl_alarm_static, OS_CONST) alarm_navigation_static = {
  {
    /* pointer to counter           */  &SystemCounter_counter_desc,
    /* pointer to the expiration    */  tpl_raise_alarm
#if (WITH_TRACE == YES)
    /* id of the alarm for tracing  */  , alarm_navigation_id
#endif
#if WITH_OSAPPLICATION == YES
    /* OS application id            */  , 
#endif
  },
  /* action of the alarm  */  (tpl_action *)&alarm_navigation_action
};
...
VAR(tpl_time_obj, OS_VAR) alarm_navigation_alarm_desc = {
    /* pointer to the static part   */  (tpl_time_obj_static *)&alarm_navigation_static,
    /* next alarm                   */  NULL,
    /* prev alarm                   */  NULL,
    /* cycle                        */  600,
    /* date                         */  100,
